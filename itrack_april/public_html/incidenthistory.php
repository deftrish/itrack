<?php
    ...
    if($_SESSION['type'] == 'itms'){
        $sql = "SELECT * from incident_mgmt left join users user2 on closed_by = user2.idUsers  join users user1 on incidentInv  = user1.idUsers where complete = 1 order by updated_at";
    }else{
        $sql = "SELECT * from incident_mgmt left join users user2 on closed_by = user2.idUsers  join users user1 on incidentInv  = user1.idUsers where complete = 1 and incidentInv = $id order by updated_at";
    }
                ...
                <tbody id="table-body">
                    ...
                    <?php }else { 
                        while($row = $result->fetch_array()){
                            ...
                            echo "<td><a class='btn btn-success btn-sm' href='reopenIncident.php?id=$id'>View Incident</a> <a class='btn btn-primary btn-sm' href='incidentDetails.php?id=$id'>View Logs</a></td>";
                            echo '</tr>';
                        }
                    } ?>
                </tbody>
                ...
                $.ajax({
                    ...
                    success: function(response){
                        ...
                        $.each(response, function(k, v){
                            ...
                            tr.append('<td><a class="btn btn-success btn-sm" href="reopenIncident.php?id='+v.incidentTicket+'">View Incident</a><a class="btn btn-primary btn-sm" href="incidentDetails.php?id='+v.incidentTicket+'">View Logs</a></td>');
